...
class Platforms:
	...
	def generate_random_coordinates(n):
		...
		original_n = n # in order to let the input stays the same we use this
		n -= 5 # we first make 3 additional spots for platforms that might be add
		...
		while len(final_choice) < original_n:
			n += 1
			if (choice[n+1][0], choice[n+1][1]) not in final_choice:
				final_choice.append((choice[n+1][0], choice[n+1][1]))

		final_choice.sort(key = lambda c: c[1], reverse = True)
		return final_choice
		...
		for platform in self._list_of_platforms:
			...
			if collide and doodle._t < 0:
				if platform._y < WIN_HEIGHT - 50:
					...
					doodle._t = 1
					doodle._t2 = 1
					return round(dy), round(score_plus)
				else:
					doodle._t = 1
				break
		return 0, 0
		...
		if self._move > 0 and self._t2 > 0:
			...
			for platform in self._list_of_platforms:
				...
				if platform._y > WIN_HEIGHT - 7.5: # if it is half gone (15 / 2)
					...
					platform._y = 7.5
					self._list_of_platforms.sort(key = lambda c: c._y, reverse = True)
